...
const DashboardProductCard = ({ product }) => {
  ...
  return (
    <div className="bg-white p-4 md:p-6 rounded-2xl shadow-lg flex items-center gap-4 md:gap-6 w-full  mx-auto">
      {/* Product Image */}
      <figure className={`bg-gray-200 rounded-xl shrink-0 overflow-hidden ${activeTab === "wishlist" ? "w-28 h-28 md:w-48 md:h-40" : "w-24 h-24 md:w-36 md:h-26"}`}>
        <img
          src={product_image}
          alt={product_title}
      ...
      <div className="flex-1 space-y-1 md:space-y-3">
        <div className="flex items-center justify-between">
          ...
          {/* Remove Button */}
          <button className="ml-auto w-8 h-8 rounded-full bg-red-100 text-red-500 flex shrink-0 items-center justify-center hover:bg-red-200 transition-colors">
            <IoClose size={20} />
          </button>
        </div>
        ...
        <p className="text-sm md:text-base font-semibold text-gray-800">Price: ${price}</p>
        {activeTab === "wishlist" && (
